...
class Tester(unittest.TestCase):
    ...
    def test_py(self):
        with open("test_data/test_output.txt", "w") as fd:
            dater = src.CommentDater("test_data/test_infile.py", 'master', output = fd)
            dater.parse()
        ...
        self.assertNotEqual(output.find("possible outdated comment at test_data/test_infile.py:9"), -1)
        
        # check that modified comments aren't included in output 
        # (file modified at line 13 and line 14)
        self.assertEqual(output.find("possible outdated comment at test_data/test_infile.py:13"), -1)
        
        # check that multiline comments are handled (file modified at line 19)
        self.assertNotEqual(output.find("possible outdated comment at test_data/test_infile.py:16"), -1)
    
    def test_c(self):
        with open("test_data/test_output.txt", "w") as fd:
            dater = src.CommentDater("test_data/test_infile.c", 'master', output = fd)
            dater.parse()
        with open("test_data/test_output.txt", "r") as fd:
            output = "".join(list(fd.readlines()))
        
        # check outdated single line comment (file modified line 3)
        self.assertNotEqual(output.find("possible outdated comment at test_data/test_infile.cc:1"), -1)
        
        # check that modified comments aren't included in output 
        # (file modified at line 6 and line 7)
        self.assertEqual(output.find("possible outdated comment at test_data/test_infile.cc:6"), -1)
